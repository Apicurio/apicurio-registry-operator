...
func (this *DeploymentOcpCF) Sense() {
	...
	deployments, err := this.ctx.RequireService(svc.SVC_CLIENTS).(Clients).OCP().GetDeployments(
...
func (this *DeploymentOcpCF) Respond() {
	...
	if this.deploymentName != RC_EMPTY_NAME {
		...
		for _, val := range this.deployments {
			if val.Name == this.deploymentName {
				contains = true
				this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(ResourceCache).Set(RC_KEY_DEPLOYMENT_OCP, NewResourceCacheEntry(val.Name, &val))
				break
			}
		}
		if !contains {
	...
	if this.deploymentName == RC_EMPTY_NAME && len(this.deployments) == 1 {
		...
		this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(ResourceCache).Set(RC_KEY_DEPLOYMENT_OCP, NewResourceCacheEntry(deployment.Name, &deployment))
	}
	// Response #3 (and #4)
	// If there is no deployment available (or there are more than 1), just create a new one
	if this.deploymentName == RC_EMPTY_NAME && len(this.deployments) != 1 {
		deployment := this.ctx.RequireService(svc.SVC_OCP_FACTORY).(OCPFactory).CreateDeployment()
		// leave the creation itself to patcher+creator so other CFs can update
		this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(ResourceCache).Set(RC_KEY_DEPLOYMENT_OCP, NewResourceCacheEntry(RC_EMPTY_NAME, deployment))
	}
}
...
func (this *DeploymentOcpCF) Cleanup() bool {
	...
	if deploymentEntry, deploymentExists := this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(ResourceCache).Get(RC_KEY_DEPLOYMENT_OCP); deploymentExists {
		if err := this.ctx.RequireService(svc.SVC_CLIENTS).(Clients).OCP().DeleteDeployment(deploymentEntry.GetValue().(*ocp_apps.DeploymentConfig), &meta.DeleteOptions{});
