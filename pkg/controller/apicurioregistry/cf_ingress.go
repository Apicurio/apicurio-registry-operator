...
func (this *IngressCF) Sense() {
	...
	ingresses, err := this.ctx.RequireService(svc.SVC_CLIENTS).(Clients).Kube().GetIngresses(
	...
	// Update the status
	this.ctx.RequireService(svc.SVC_CONFIGURATION).(Configuration).SetConfig(CFG_STA_INGRESS_NAME, this.ingressName)
}
...
func (this *IngressCF) Respond() {
	...
	if this.ingressName != RC_EMPTY_NAME {
		...
		for _, val := range this.ingresses {
			if val.Name == this.ingressName {
				contains = true
				this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(ResourceCache).Set(RC_KEY_INGRESS, NewResourceCacheEntry(val.Name, &val))
				break
			}
		}
		if !contains {
	...
	if this.ingressName == RC_EMPTY_NAME && len(this.ingresses) == 1 {
		...
		this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(ResourceCache).Set(RC_KEY_INGRESS, NewResourceCacheEntry(ingress.Name, &ingress))
	}
	// Response #3 (and #4)
	// If there is no ingress available (or there are more than 1), just create a new one
	if this.ingressName == RC_EMPTY_NAME && len(this.ingresses) != 1 {
		ingress := this.ctx.RequireService(svc.SVC_KUBE_FACTORY).(KubeFactory).CreateIngress(this.serviceName)
		// leave the creation itself to patcher+creator so other CFs can update
		this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(ResourceCache).Set(RC_KEY_INGRESS, NewResourceCacheEntry(RC_EMPTY_NAME, ingress))
	}
}

func (this *IngressCF) Cleanup() bool {
	// Ingress should not have any deletion dependencies
	if ingressEntry, ingressExists := this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(ResourceCache).Get(RC_KEY_INGRESS); ingressExists {
		if err := this.ctx.RequireService(svc.SVC_CLIENTS).(Clients).Kube().DeleteIngress(ingressEntry.GetValue().(*extensions.Ingress), &meta.DeleteOptions{});
