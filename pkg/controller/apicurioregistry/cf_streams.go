...
func (this *StreamsCF) Sense() {
	...
	_, exists := this.ctx.RequireService(svc.SVC_ENV_CACHE).(EnvCache).Get(ENV_APPLICATION_SERVER_HOST)
	this.envApplicationServerHostExists = exists
	if val, exists := this.ctx.RequireService(svc.SVC_ENV_CACHE).(EnvCache).Get(ENV_APPLICATION_SERVER_PORT); exists {
		this.envApplicationServerPort = val.GetValue().Value
	}
	if val, exists := this.ctx.RequireService(svc.SVC_ENV_CACHE).(EnvCache).Get(ENV_APPLICATION_ID); exists {
...
func (this *StreamsCF) Respond() {
	...
	this.ctx.RequireService(svc.SVC_ENV_CACHE).(EnvCache).Set(NewSimpleEnvCacheEntry(ENV_APPLICATION_SERVER_PORT, this.applicationServerPort))
	this.ctx.RequireService(svc.SVC_ENV_CACHE).(EnvCache).Set(NewSimpleEnvCacheEntry(ENV_APPLICATION_ID, this.applicationId))
