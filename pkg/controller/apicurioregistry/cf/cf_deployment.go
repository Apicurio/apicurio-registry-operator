...
func (this *DeploymentCF) Sense() {
	...
	deployments, err := this.ctx.RequireService(svc.SVC_CLIENTS).(*client.Clients).Kube().GetDeployments(
...
func (this *DeploymentCF) Respond() {
	...
	if this.deploymentName != resources.RC_EMPTY_NAME {
		...
		for _, val := range this.deployments {
			if val.Name == this.deploymentName {
				contains = true
				this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(resources.ResourceCache).Set(resources.RC_KEY_DEPLOYMENT, resources.NewResourceCacheEntry(val.Name, &val))
				break
			}
		}
		if !contains {
	...
	if this.deploymentName == resources.RC_EMPTY_NAME && len(this.deployments) == 1 {
		...
		this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(resources.ResourceCache).Set(resources.RC_KEY_DEPLOYMENT, resources.NewResourceCacheEntry(deployment.Name, &deployment))
	}
	// Response #3 (and #4)
	// If there is no deployment available (or there are more than 1), just create a new one
	if this.deploymentName == resources.RC_EMPTY_NAME && len(this.deployments) != 1 {
		deployment := this.ctx.RequireService(svc.SVC_KUBE_FACTORY).(*factory.KubeFactory).CreateDeployment()
		// leave the creation itself to patcher+creator so other CFs can update
		this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(resources.ResourceCache).Set(resources.RC_KEY_DEPLOYMENT, resources.NewResourceCacheEntry(resources.RC_EMPTY_NAME, deployment))
	}
}
...
func (this *DeploymentCF) Cleanup() bool {
	...
	if deploymentEntry, deploymentExists := this.ctx.RequireService(svc.SVC_RESOURCE_CACHE).(resources.ResourceCache).Get(resources.RC_KEY_DEPLOYMENT); deploymentExists {
		if err := this.ctx.RequireService(svc.SVC_CLIENTS).(*client.Clients).Kube().DeleteDeployment(deploymentEntry.GetValue().(*apps.Deployment), &meta.DeleteOptions{});
