...
* Docker
* https://github.com/golang/go[go] (1.15+, with `export GO111MODULE='on'`), and `$GOPATH` and `$GOROOT` set.
...
`$ cat config/examples/resources/registry_v2_apicurioregistry.yaml | kubectl -n system apply -f -`
...
1. Run `$ cat config/examples/resources/registry_v2_apicurioregistry.yaml | kubectl -n system delete -f -; make undeploy`
